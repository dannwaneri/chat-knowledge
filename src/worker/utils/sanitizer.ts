...
export class ChatSanitizer {
  private patterns: SensitivePattern[] = [
    // API Keys — specific patterns first, before generic
    {
      type: 'api_key',
  ...
  async scanChat(messages: Array<{ speaker: string; content: string }>): Promise<SanitizationReport> {
    ...
    messages.forEach((msg, index) => {
      ...
      this.patterns.forEach((pattern) => {
        // FIX: Reset lastIndex before each use to avoid g-flag state pollution
    ...
    return messages.map((msg) => {
      ...
      this.patterns.forEach((pattern) => {
        ...
        if (pattern.severity === 'critical') {
          ...
          // FIX: Reset lastIndex before each use to avoid g-flag state pollution
          pattern.pattern.lastIndex = 0;
          const newContent = content.replace(pattern.pattern, replacement);
          // FIX: Only set redacted=true when content actually changed
  ...
  private generateWarnings(detected: DetectedIssue[]): string[] {
    ...
    if (criticalCount > 0) {
      warnings.push(
        `CRITICAL: Found ${criticalCount} sensitive items (API keys, passwords, tokens). These will be auto-redacted.`
      );
    }

    const privateUrls = detected.filter(d => d.type === 'private_url').length;
    if (privateUrls > 0) {
      warnings.push(`Found ${privateUrls} internal/private URLs. Review before sharing.`);
    }

    const emails = detected.filter(d => d.type === 'email').length;
    if (emails > 0) {
      warnings.push(`Found ${emails} email addresses. Consider if they should be public.`);
    }
  ...
  generateUIReport(report: SanitizationReport): string {
    ...
    lines.push('='.repeat(60));
    lines.push('');
    lines.push(`Status: ${report.safe ? 'SAFE TO SHARE' : 'ISSUES FOUND'}`);
      ...
      Object.entries(bySeverity).forEach(([severity, issues]) => {
        if (issues.length > 0) {
          ...
          issues.forEach((issue, i) => {
            ...
            if (issue.autoRedact) lines.push(`     Will be auto-redacted`);
            lines.push('');
          });
        }
      });
...
export async function scanBeforeSharing(
  ...
  return { safe: report.safe, report, redactedMessages };
}